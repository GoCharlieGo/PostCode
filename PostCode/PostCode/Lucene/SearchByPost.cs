...
namespace PostCode.Lucene
{
    public class SearchPublication
    {
        ...
        private const string TextName = "Content";
        ...
        private void IndexCreate()
        {
            ...
            try
            {
                ...
                {
                    foreach (var publication in _context.Posts.ToList())
                    {
        ...
        public IEnumerable<Post> Find(string query)
        {
            // 1.
        ...
        private IEnumerable<KeyValuePair<int, Post>> FindDocumentPublication(string query, Searcher searcher)
        {
            // 1.
            ...
            // 3.
            var documentBookIds = scoreDocs.Select(x => new KeyValuePair<int, string>
            (
              x.Doc,
              searcher.Doc(x.Doc).Get(IdName)
            )).ToList();
